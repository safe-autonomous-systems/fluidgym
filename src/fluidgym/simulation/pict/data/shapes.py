...
def rotate_grid(
    ...
    if distance_scaling is not None:
        ...
        make_matrix_rotation = (
            make_matrix_rotation_2D
            if dims == 2
            else lambda angle: make_matrix_rotation_3D(angle * axis)
        )
        rotation_matrices = np.asarray([make_matrix_rotation(a) for a in angle])

        rotation_matrices = torch.tensor(
            rotation_matrices, device=grid.device, dtype=grid.dtype
        )
        ...
        rotation_matrix = torch.tensor(
            rotation_matrix, device=grid.device, dtype=grid.dtype
        )
...
def get_grid_normal_dist_from_ortho_transforms(
    transforms, mean, var, dtype=torch.float32, normalize=True
):
...
def interpolate_vertices_from_borders_2D(
    borders, x_weights=None, y_weights=None, dtype=torch.float32
):
    # borders: 2D: [-x,+x,-y,+y]
...
def extrapolate_boundary_layers(grid, boundaries=[]):
    ...
    assert (
        isinstance(grid, torch.Tensor) and grid.dim() == 4
...
def make_cosX_grid(x, y, x_scale=1, y_scale=1, strength=1):
    ...
    offsets = np.asarray(
        [
            ((np.cos(x_norm * _) + 1) * 0.5 * x_scale * strength, _ * y_scale)
            for _ in range(y + 1)
        ]
    )
    ...
    vertex_coords = torch.tensor(
        vertex_coords, dtype=dtype, device=cuda_device
    ).contiguous()
