...
class WelfordOnlineParallel_Torch:
    ...
    def __init__(self, dims, record_steps=False, squeeze_dims=True):
        ...
        if not (
            ...
            and all(isinstance(dim, int) for dim in dims)
            and (
                len(dims) < 2
                or all(dims[i] < dims[i + 1] for i in range(len(dims) - 1))
            )
        ):
    ...
    def update(self, n_b, mean_b, sum_squares_b, record_step=True):
        ...
        sum_squares = (
            self.sum_squares + sum_squares_b + torch.square(delta) * (self.n * n_b / n)
        )
    ...
    def update_from_data(self, data, record_step=True):
        ...
        sum_squares = torch.sum(
            torch.square(data - mean), dim=self.dims, keepdim=not self._squeeze_dims
        )
    ...
    def load(self, path, start=0, end=None, device=None, dtype=torch.float32):
        with np.load(path) as np_file:
            ...
            self.sum_squares = torch.tensor(
                np_file["sum_squares"], device=device, dtype=dtype
            )

            if self.record_steps:
                if "steps_n" not in np_file:
        ...
        if self.record_steps and (start != 0 or end != total_steps):
            ...
            for step in range(len(self.steps_n)):
                self.update(
                    torch.tensor(self.steps_n[step], device=device, dtype=dtype),
                    torch.tensor(self.steps_mean[step], device=device, dtype=dtype),
                    torch.tensor(
                        self.steps_sum_squares[step], device=device, dtype=dtype
                    ),
                    record_step=False,
                )
...
class CovarianceOnlineParallel_Torch:
    ...
    def update(self, n_b, mean_x_b, mean_y_b, C_b, record_step=True):
        ...
        C = (
            self.C
            + C_b
            + (self.mean_x - mean_x_b) * (self.mean_y - mean_y_b) * self.n * n_b / n
        )
    ...
    def update_from_data(self, data_x, data_y, record_step=True):
        ...
        C = torch.sum(
            (data_x - mean_x) * (data_y - mean_y),
            dim=self.dims,
            keepdim=not self._squeeze_dims,
        )
        if self._squeeze_dims:
...
class PSDOnline_Torch:
    def __init__(
        ...
        if not (
            isinstance(fft_dims, (list, tuple))
            and all(isinstance(_, int) and _ < total_dims for _ in fft_dims)
        ):
        ...
        if planes is not None:
            if not (
                isinstance(planes, (list, tuple))
                and all(isinstance(_, int) for _ in planes)
            ):
                raise TypeError("'planes' must be a list of int")
            if not (len(planes) > 0 and len(set(planes)) == len(planes)):
                raise ValueError(
                    "at least one plane must be provided and all planes must be distinct"
                )
...
class MultivariateMomentsOnlineParallel_Torch:
    ...
    class MultivariateMomentsData:
        ...
        def from_data(cls, moments, data_list, avg_dims):
            if not (
                isinstance(data_list, (list, tuple))
                and all(isinstance(data, torch.Tensor) for data in data_list)
            ):
            ...
            if any(data.size() != shape for data in data_list):
                raise ValueError(
                    "All data tensors must have same shape. Shapes: %s"
                    % ([data.shape for data in data_list],)
                )
            ...
            self.means = [
                torch.mean(data, dim=avg_dims, keepdim=True) for data in data_list
            ]
            ...
            for moment in moments:
                ...
                self.moments[moment] = torch.sum(
                    moment_data, dim=avg_dims, keepdim=True
                )
        ...
        def set_data(self, n, means, moments):
            if not (
                ...
                and all(isinstance(mean, torch.Tensor) for mean in means)
            ):
                raise TypeError("means must be a list of torch.Tensor.")
            if not len(means) == self.channels:
                raise ValueError(
                    "MultivariateMomentsData has %d channels but got %d means."
                    % (self.channels, len(means))
                )

            if not (
                isinstance(moments, dict)
                and not all(
                    isinstance(moment_data, torch.Tensor)
                    for moment_data in moments.items()
                )
            ):
        ...
        def from_file(cls, path, start=0, end=None, device=None, dtype=torch.float32):
            if not (start == 0 and (end is None)):
                raise NotImplementedError(
                    "Sliced loading is not supported: [%s:%s]" % (start, end)
                )  # needs step recording
            ...
            with np.load(path) as np_file:
                ...
                means = [
                    torch.tensor(np_file["mean_%06d" % idx], device=device, dtype=dtype)
                    for idx in range(num_means)
                ]
                ...
                for key in np_file.keys():
                    if key.startswith("moment_"):
                        moment_keys.add(
                            (key, tuple(int(channel) for channel in key[7:].split("_")))
                        )

                if not len(moment_keys) == num_moments:
                    raise IOError(
                        "Expected %d moments, but found %d: %s"
                        % (num_moments, len(moment_keys), [_[1] for _ in moment_keys])
                    )

                moments = {
                    moment_key: torch.tensor(np_file[key], device=device, dtype=dtype)
                    for key, moment_key in moment_keys
                }
    ...
    def _update(self, other):
        means_diff = [
            self.data.get_mean(channel) - other.get_mean(channel)
        ...
        means_merged = [
            (
                self.data.get_mean(channel) * self.data.n
                + other.get_mean(channel) * other.n
            )
            * r_n_merged
            for channel in range(self.max_channels)
        ]
        ...
        for moment in self.moments:
            ...
            for beta, alpha_minus_beta in zip(*self._get_betas(moment)):
                if sum(beta) == 0:
                    ...
                    moment_data_merged = (
                        moment_data_merged
                        + self.data.get_moment(alpha_minus_beta)
                        + other.get_moment(alpha_minus_beta)
                    )
                    ...
                    moment_data_merged = moment_data_merged + binom_factor * (
                        ((-other.n * r_n_merged) ** exp)
                        * self.data.get_moment(alpha_minus_beta)
                        + ((self.data.n * r_n_merged) ** exp)
                        * other.get_moment(alpha_minus_beta)
    ...
    def update_from_data(self, data_list):
        ...
        other = (
            MultivariateMomentsOnlineParallel_Torch.MultivariateMomentsData.from_data(
                self.moments, data_list, self.avg_dims
            )
        )
    ...
    def get_moment_standardized(
        self, i, order, to_wall_fn=lambda x, o: x, squeeze=True
    ):
        moment_key = [0] * self.max_channels
        moment_key[i] = order
        return to_wall_fn(self.get_moment_normalized(moment_key), order) / (
            to_wall_fn(self.get_moment_normalized(self._get_moment_key(i, i)), 2)
            ** (order / 2)
        )
    ...
    def load(self, path, start=0, end=None, device=None, dtype=torch.float32):
        new_data = (
            MultivariateMomentsOnlineParallel_Torch.MultivariateMomentsData.from_file(
                path, start=start, end=end, device=device, dtype=dtype
            )
        )
        if not (
            new_data.channels == self.max_channels
            and new_data.get_moment_keys() == self.moments
        ):
    ...
    def from_file(
        ...
        if not (data.channels == m.max_channels and m.moments == moments):
            raise RuntimeError(
                "Error while loading moments data:\navailable moments: %s\nrequired moments: %s"
                % (moments, m.moments)
            )
        m.data = data
        return m
...
class TurbulentEnergyBudgetsOnlineParallel_Torch:
    def __init__(
        self, avg_dims=[0, 2], grid_coordinates=None, with_forcing=False, u_wall=None
    ):
        ...
        self.dims = 3
        if not (
            isinstance(avg_dims, (list, tuple))
            and len(set(avg_dims)) == len(avg_dims)
            and all(0 <= dim and dim < self.dims for dim in avg_dims)
        ):
        ...
        variance_moments = list(
            filter(
                lambda moment: sum(moment) in [2, 3],
                itertools.product((0, 1, 2, 3), repeat=self.dims),
            )
        )
        variance_moments = [
            var_moment + (0, 0, 0) for var_moment in variance_moments
        ]  # pad pressure gradient channels

        pressure_variance_moments = list(
            filter(
                lambda moment: sum(moment) == 1,
                itertools.product((0, 1), repeat=self.dims),
            )
        )  # all tuples of len dims with one 1
        pressure_variance_moments = list(
            itertools.product(pressure_variance_moments, repeat=2)
        )  # any combination of these tuples
        pressure_variance_moments = [
            vel + p for vel, p in pressure_variance_moments
        ]  # concatenate tuples
        ...
        if with_forcing:
            variance_moments = [
                moment + (0, 0, 0) for moment in variance_moments
            ]  # channels: u, v, w, px, py, pz, sx, sy, sz

            velocity_forcing_moments = list(
                filter(
                    lambda moment: sum(moment) == 1,
                    itertools.product((0, 1), repeat=self.dims),
                )
            ...
            velocity_forcing_moments = list(
                itertools.product(velocity_forcing_moments, repeat=2)
            )  # any combination of these tuples
            velocity_forcing_moments = [
                vel + (0, 0, 0) + p for vel, p in velocity_forcing_moments
            ]  # concatenate tuples, pad unused pressure

            variance_moments = variance_moments + velocity_forcing_moments
            del velocity_forcing_moments

        self.moments_data = MultivariateMomentsOnlineParallel_Torch(
            variance_moments, self.dims * 2, self.avg_dims
        )

        # (co)variances
        variance_moments = list(
            filter(
                lambda moment: sum(moment) == 2,
                itertools.product((0, 1, 2), repeat=self.dims),
            )
        )
        self.moments_data_grad = [
            # ux, vx, wx
            MultivariateMomentsOnlineParallel_Torch(
                variance_moments, self.dims, self.avg_dims
            ),
            # uy, vy, wy
            MultivariateMomentsOnlineParallel_Torch(
                variance_moments, self.dims, self.avg_dims
    ...
    def update_from_data(self, vel_data, p_data, s_data=None):
        ...
        p_grads = tuple(
            self._data_grad(p_data[0, 0], grad_dim) for grad_dim in range(self.dims)
        )
        ...
        for grad_dim in range(self.dims):
            self.moments_data_grad[grad_dim].update_from_data(
                torch.unbind(self._data_grad(vel_data, grad_dim)[0], dim=0)
            )
    ...
    def _data_grad(self, data, grad_dim, borders="ZERO"):
        ...
        if not all(
            pos.size(-dim - 1) == data.size(-dim - 1) for dim in range(self.dims)
        ):
            raise ValueError(
                "grid and data sizes do not match: %s, %s" % (pos.size(), data.size())
            )
        ...
        if borders == "ONESIDED":
            ...
            data_grad = torch.cat(
                [data_grad_lower, data_grad, data_grad_upper], dim=grad_dim
            )
            if self.grid_coordinates is not None:
                raise NotImplementedError
        elif borders == "PAD":
            ...
            slice_upper[grad_dim] = slice(dim_size - 1, dim_size)
            data_grad = torch.cat(
                [data_grad[slice_lower], data_grad, data_grad[slice_upper]],
                dim=grad_dim,
            )
    ...
    def _data_grad2(self, data, grad_dim, borders="ZERO"):
        ...
        if self.grid_coordinates is not None:
            data_grad_lower = data_grad_lower / torch.abs(
                pos[slice_mid] - pos[slice_lower]
            )
            data_grad_upper = data_grad_lower / torch.abs(
                pos[slice_upper] - pos[slice_mid]
            )
        ...
        if self.grid_coordinates is not None:
            data_grad = data_grad / (
                torch.abs(pos[slice_upper] - pos[slice_lower]) * 0.5
            )
    ...
    def production(self, i, j, as_wall=True):
        ...
        for k in range(self.dims):
            if k not in self.avg_dims:  # self.mean_grad() would be 0
                result = result - self.covariance(
                    i, k, as_wall=as_wall
                ) * self.mean_grad(j, k, as_wall=as_wall)
                result = result - self.covariance(
                    j, k, as_wall=as_wall
                ) * self.mean_grad(i, k, as_wall=as_wall)
    ...
    def turbulent_transport(self, i, j, as_wall=True):
        ...
        for k in range(self.dims):
            if k not in self.avg_dims:
                result = result - self._data_grad(
                    self.skewness(i, j, k, squeeze=False, as_wall=as_wall), k
                )
    ...
    def viscous_diffusion(self, i, j, as_wall=True):
        ...
        for k in range(self.dims):
            if k not in self.avg_dims:
                result = result + self._data_grad2(
                    self.covariance(i, j, squeeze=False, as_wall=as_wall), k
                )
    ...
    def velocity_pressure_gradient(self, i, j, as_wall=True):
        result = -(
            self.covariance(i, j + 3, as_wall=as_wall)
            + self.covariance(j, i + 3, as_wall=as_wall)
        )
    ...
    def velocity_forcing(self, i, j, as_wall=True):
        ...
        result = self.covariance(i, j + 6, as_wall=as_wall) + self.covariance(
            j, i + 6, as_wall=as_wall
        )
    ...
    def save(self, path, name="budgets", save_steps=True):
        ...
        self.moments_data.save(
            os.path.join(path, "%s_moments.npz" % (name,)), save_steps=save_steps
        )
        for i, moments_grad in enumerate(self.moments_data_grad):
            moments_grad.save(
                os.path.join(path, "%s_grad_%04d.npz" % (name, i)),
                save_steps=save_steps,
            )

    def load(
        self, path, name="budgets", start=0, end=None, device=None, dtype=torch.float32
    ):
        if not (start == 0 and (end is None)):
            raise NotImplementedError(
                "Sliced loading [%s:%s] is not supported." % (start, end)
            )  # needs step recording

        self.moments_data.load(
            os.path.join(path, "%s_moments.npz" % (name,)),
            start=start,
            end=end,
            device=device,
            dtype=dtype,
...
class TemporalTwoPointCorrelation_Online_torch:
    ...
    def _init_base(self, fluctuations):
        ...
        self.base_rms = torch.sqrt(
            torch.mean(
                torch.square(self.base_fluctuations),
                dim=self.dims,
                keepdim=not self._squeeze_dims,
            )
        )
    ...
    def update(self, fluctuations, time):
        ...
        mean_squares = torch.mean(
            self.base_fluctuations * fluctuations,
            dim=self.dims,
            keepdim=not self._squeeze_dims,
        )
        rms = torch.sqrt(
            torch.mean(
                torch.square(fluctuations),
                dim=self.dims,
                keepdim=not self._squeeze_dims,
            )
        )
    ...
    def load(self, path, start=0, end=None, device=None, dtype=torch.float32):
        with np.load(path) as np_file:
            self.base_fluctuations = torch.tensor(
                np_file["base_fluctuations"], device=device, dtype=dtype
            )
            self.base_rms = torch.tensor(
                np_file["base_rms"], device=device, dtype=dtype
            )
