...
class TorrojaProfile:
    ...
    def __init__(self, base_path, Re):
        ...
        with open(path, "r") as profiles_file:
            ...
            self.profiles = {
                ...
                for name, values in zip(self.field_names, stats)
            }
    ...
    def plot_full_stats(self, path, file_type="svg"):
        ...
        fig.savefig(
            os.path.join(
                path, "Reference_profiles_full_Re%d.%s" % (self.Re_wall, file_type)
            )
        )
        plt.close(fig)
        plt.clf()
    ...
    def plot_stats(self, path, use_y_wall_units=False, file_type="svg"):
        ...
        fig.tight_layout()
        fig.savefig(
            os.path.join(path, "Reference_profiles_Re%d.%s" % (self.Re_wall, file_type))
        )
        plt.close(fig)
        plt.clf()
...
class TorrojaBalances:
    ...
    class TorrojaBalance:
        ...
        def __init__(self, base_path, Re, i, j):
            path = os.path.join(
                base_path,
                "Re%d.bal.%s%s" % (Re, self._components[i], self._components[j]),
            )
        ...
        def get_full_pos_y(self):
            ...
            return np.concatenate(
                (self.balances[key] - 1, 1 - self.balances[key][::-1])
            )
...
class TorrojaSpectra:
    ...
    def __init__(self, base_path, Re):
        ...
        for jind in range(self.nplan):
            with open(
                os.path.join(base_path, "Re%s.spe.j%02d" % (Re, jind + 1)), "rb"
            ) as file:
    ...
    def _load_data_record(self, file, jind, data_type):
        ...
        file.read(4)
        self.spectra[data_type][jind] = np.frombuffer(
            file.read(buffer_size), dtype="<f4"
        ).reshape(nz1, nx1)
        file.read(4)
    ...
    def plot_spectra(
        self, path, file_type="svg", jinds=None, data_types=None, log_fn=None
    ):
        if jinds is None:
        ...
        if log_fn is not None:
            log_fn(
                "lstar=%s", lstar
            )  # , lambdax=%s, lambdaz=%s", lstar, lambdax, lambdaz)

        for col, data_type in enumerate(data_types):
            for row, jind in enumerate(jinds):
                if log_fn is not None:
                    log_fn("plotting '%s' for j=%d", data_type, jind + 1)
                data = self.spectra[data_type][jind]
                if data is None:
                    continue
                data = kxx * kzz * data
                ...
                if log_fn is not None:
                    log_fn(
                        "normalized data stats: mean %s, min %s, max %s",
                        np.mean(data),
                        np.min(data),
                        np.max(data),
                    )
                ...
                ax = axs[row][col]
                ax.contourf(
                    lambdax, lambdaz, data, levels=[0.1, 0.5, 0.9, 1], cmap="Greys"
                )
        ...
        fig.tight_layout()
        fig.savefig(
            os.path.join(path, "Reference_spectra_Re%d.%s" % (self.Re_wall, file_type))
        )
        plt.close(fig)
        plt.clf()
