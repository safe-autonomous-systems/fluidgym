...
class Profiler:
    ...
    def _print_stats(
        self, sample, level, t_parent, t_root, file, level_indent, max_indent
    ):
        ...
        for name, current in sorted(
            ...
            file.write(
                "{}: {:>10}, {:10d}, {:>12}, {: 10.05f}, {: 10.05f}, {:>10}, {:>10}, {:>10}\n".format(
                    s,
                    time_unit(current.mean),
                    current.num_samples,
                    format_time(total_time),
                    100.0 * total_time / t_parent,
                    100.0 * total_time / t_root,
                    time_unit(current.std),
                    time_unit(current.min),
                    time_unit(current.max),
                )
            )
            self._print_stats(
