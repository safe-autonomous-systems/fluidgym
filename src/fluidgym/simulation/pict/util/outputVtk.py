...
try:  # for compatibility with existing setups that do not include pyvista
    ...
    def save_vtk(domain: PISOtorch.Domain, out_dir, it, vertex_coord_list=None):
        ...
        assert vertex_coord_list is not None
        ...
        for blockIdx, vertex_coords in enumerate(vertex_coord_list):
            ...
            index_tensor = (
                torch.linspace(start_index, start_index + numel - 1, numel)
            ...
            points_list.append(
                vertex_coords.reshape(vertex_coords.shape[1], -1).permute(1, 0)
            )
            ...
            cell_indices = (
                torch.stack(
                    torch.meshgrid(
                        *[torch.linspace(0, c - 1, c) for c in cells_shape],
                        indexing="ij",
                    )
                )
                .to(torch.int32)
            ...
            corner_indices = (
                PISOtorch.GetElementCornerValues(
                    index_tensor.unsqueeze(0).unsqueeze(0).to(torch.float32).to("cuda"),
                    cell_indices,
                )
                .cpu()
                .squeeze(0)
        ...
        if ndims == 2:
            all_vertices = torch.cat(
                [all_vertices, torch.zeros(all_vertices.shape[0], 1)], dim=-1
            )
        ...
        if ndims == 2:
            cell_types = np.array(
                [pv.CellType.QUAD for i in range(int(all_cells.shape[0] / 5))],
                dtype=np.uint8,
            )
        else:
            cell_types = np.array(
                [pv.CellType.HEXAHEDRON for i in range(int(all_cells.shape[0] / 9))],
                dtype=np.uint8,
            )
        ...
        for blockIdx, vertex_coords in enumerate(vertex_coord_list):
            ...
            if ndims == 2:
                ...
                velocity_tensor = torch.cat(
                    [velocity_tensor, torch.zeros(velocity_tensor.shape[0], 1)], dim=-1
                )
            else:
                velocity_tensor = (
                    block.velocity.cpu()
