...
def pad_to_size(data, size_x, size_y, padding=0, pad_col=(0,)):
    ...
    data = [
        torch.nn.functional.pad(data[..., i], paddings, value=pad_col[i])
    ...
    for row_idx, row in enumerate(layout):
        ...
        for col_idx, block_idx in enumerate(row):
            ...
            if block_idx == -1:
                ...
                row_data.append(
                    pad_to_size(blocks[block_idx], size_x, size_y, padding, pad_col)
                )
...
def save_block_data_image(
    ...
    axis3D=0,
    mode3D="slice",
    vertex_coord_list=None,
    resampling_out_shape=10,
    fill_max_steps=0,
    image_format="PNG",
):
    if not (
    ...
    if not all(
        ...
        for data, block in zip(block_data, domain.getBlocks())
    ):
...
def save_scalar_image(
    ...
    if vertex_coord_list is None or resampling_out_shape is None:
        ...
        block_data = arrange_blocks(
            block_data, layout=layout, padding=padding, pad_col=pad_col
        )
